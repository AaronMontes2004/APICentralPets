import { addSaleDetailQuery, findByIdSaleDetailQuery, getSalesDetailQuery, updateSaleDetailQuery } from './../libs/queries/saleDetailQuery';
...
export const findByIdSaleDetail = async(req: Request,res: Response): Promise<Response> => {
    try {
        ...
        const saleDetailObtained: any = await pool.query(findByIdSaleDetailQuery, [numSale])

        return res.status(200).json({
            status: "OK",
            msg: "Se obtuvo el detalle de la venta exitosamente",
            data: saleDetailObtained[0][0]
        })
