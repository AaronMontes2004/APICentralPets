import { addTreatmentQuery, findByIdTreatmentQuery, getTreatmentQuery, updateTreatmentQuery } from './../libs/queries/treatmentQuery';
...
export const findByIdTreatment = async(req: Request,res: Response) => {
    try {
        ...
        const treatmentObtained: any = await pool.query(findByIdTreatmentQuery, [idTreatment])

        return res.status(200).json({
            status: "OK",
            msg: "Se obtuvo el tratamiento exitosamente",
            data: treatmentObtained[0][0]
        })
